...
# from regulator2 import regulator
...
######################
    ...
    def addDirection(self, quat, pindex):
        # print (p)
        mod1 = pindex

        porig = [0,0.1,0]
        porig = mq.quaternionRotate(quat,porig)
        # print (
        # self.data[pindex]['a_position'])
        porig[0]+=self.data[pindex]['a_position'][0]
        porig[1]+=self.data[pindex]['a_position'][1]
        porig[2]+=self.data[pindex]['a_position'][2]
        # print (porig)
        ...
        dataapp = np.zeros(n, [('a_position', np.float32, 3),
                            ...
                            ('a_fg_color', np.float32, 4),
                            ('a_size', np.float32, 1)])
        dataapp['a_position'] = np.array(porig)
        # print (dataapp['a_position'])
        # dataapp['a_bg_color'] = np.append(np.random.uniform(0.8, 0.95, (n, 3)), np.array(1))
        dataapp['a_bg_color'] = 1,1,0,1
        dataapp['a_fg_color'] = 0, 0, 0, 1
        dataapp['a_size'] = [3]*n
        self.data = np.append(self.data, dataapp)

        self.edges.append([mod1, len(self.data)-1])